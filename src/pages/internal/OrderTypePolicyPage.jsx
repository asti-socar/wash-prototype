...
import {
  Plus, X, ArrowUpDown, FileDown, HelpCircle, Pencil, Trash2
} from 'lucide-react';
...
// ============== SaveConfirmModal ==============
function OTPSaveConfirmModal({ open, onClose, onConfirm, title, changes }) {
  if (!open || !changes) return null;
  const formatVal = (v) => (v === null || v === undefined) ? "없음" : Array.isArray(v) ? v.join(', ') || "없음" : String(v);
  return (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/30" onClick={onClose} />
      <div className="relative z-[1101] w-full max-w-sm bg-white rounded-xl shadow-2xl">
        <div className="px-5 py-4 border-b border-[#DFE1E6]"><h3 className="text-sm font-bold text-[#172B4D]">{title}</h3></div>
        ...
        <div className="flex h-[72px] items-center justify-end gap-2 border-t border-[#DFE1E6] px-5 bg-[#F4F5F7] rounded-b-xl">
          <button className="px-4 py-2 text-sm font-medium text-[#6B778C] bg-white border border-[#DFE1E6] rounded-lg hover:bg-gray-50" onClick={onClose}>취소</button>
          <button className="px-4 py-2 text-sm font-medium text-white bg-[#0052CC] rounded-lg hover:bg-[#0747A6]" onClick={() => { onConfirm(); onClose(); }}>확인</button>
        </div>
      </div>
    </div>
  );
...
function OTPDeleteConfirmModal({ open, onClose, onConfirm, policy }) {
  if (!open || !policy) return null;
  return (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/30" onClick={onClose} />
      <div className="relative z-[1101] w-full max-w-sm bg-white rounded-xl shadow-2xl">
        <div className="px-5 py-4 border-b border-[#DFE1E6]"><h3 className="text-sm font-bold text-[#172B4D]">정책 삭제</h3></div>
        <div className="px-5 py-4">
          <p className="text-sm text-[#172B4D]">
            <b>[ID: {policy.id}] {policy.order_type_name || policy.order_type}</b> 정책을 삭제하시겠습니까?<br />
            <span className="text-[#6B778C]">삭제된 정책은 복구할 수 없습니다.</span>
          </p>
        </div>
        <div className="flex h-[72px] items-center justify-end gap-2 border-t border-[#DFE1E6] px-5 bg-[#F4F5F7] rounded-b-xl">
          <button className="px-4 py-2 text-sm font-medium text-[#6B778C] bg-white border border-[#DFE1E6] rounded-lg hover:bg-gray-50" onClick={onClose}>취소</button>
          <button className="px-4 py-2 text-sm font-medium text-white bg-rose-600 rounded-lg hover:bg-rose-700" onClick={() => { onConfirm(policy); onClose(); }}>삭제</button>
        </div>
      </div>
    </div>
  );
...
const DetailDrawer = ({ policy, onClose, onSave, onDelete, policies, mode }) => {
  ...
  const handleSave = () => {
    ...
    setSaveConfirm({ changes, finalData });
  };
  const confirmSave = () => {
    if (!saveConfirm) return;
    const { changes, finalData } = saveConfirm;
  ...
  return (
    <><Drawer open={!!policy} title={isCreating ? "발행 유형 추가" : `[ID: ${policy.id}] 발행 유형 정책 ${isEditing ? "수정" : "상세"}`} onClose={onClose} footer={
      <div className="flex w-full items-center justify-between">
        ...
        {isEditing ? (
          ...
          <div className="flex gap-2">
            ...
            <Button className="bg-rose-600 hover:bg-rose-700 text-white" onClick={() => onDelete(policy)}>
      ...
      <Tabs>
        ...
        <TabsContent value="details" currentValue={activeTab}>
            <div className="space-y-4">
                <div>
                    <div className="text-lg font-bold text-[#172B4D]">{formData.order_type_name || '-'}</div>
                    <div className="text-sm text-slate-500 mt-1">{formData.description || '-'}</div>
                </div>
                <Card>
...
export default function OrderTypePolicyPage() {
  ...
  const handleEdit = (policy) => setDrawerState({ open: true, mode: 'view', policy });
  ...
  const handleDelete = (policy) => {
    ...
    setDrawerState({ open: false, mode: 'view', policy: null });
  };
  const handleDownloadCsv = () => {
    ...
    const columns = [
      ...
      { key: '_actions', header: '', align: 'center', render: (row) => (
        <div className="flex items-center gap-1">
          <button className="p-1.5 rounded-lg hover:bg-gray-100 transition" onClick={(e) => { e.stopPropagation(); setDrawerState({ open: true, mode: 'edit', policy: row }); }} title="수정"><Pencil className="h-4 w-4 text-[#6B778C]" /></button>
          <button className="p-1.5 rounded-lg hover:bg-rose-50 transition text-rose-500" onClick={(e) => { e.stopPropagation(); setDeleteTarget(row); }} title="삭제"><Trash2 className="h-4 w-4" /></button>
        </div>
      )}
    ];
  return (
    <div className="space-y-4">
      ...
      {drawerState.open && <DetailDrawer policy={drawerState.policy} policies={policies} mode={drawerState.mode} onClose={() => setDrawerState({ open: false, mode: 'view', policy: null })} onSave={handleSave} onDelete={(p) => { setDrawerState({ open: false, mode: 'view', policy: null }); setDeleteTarget(p); }}/>}
      <OTPDeleteConfirmModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} policy={deleteTarget} />
    </div>
  );
}
