...
export default function PartnersPage() {
  ...
  const columns = [
    ...
    { key: 'contractStatus', header: '계약 상태', render: r => <Badge tone={getStatusTone(r.contractStatus)}>{r.contractStatus}</Badge> },
    { key: 'phone', header: '연락처' },
    { key: 'assignedZoneIds', header: '배정 존 개수', render: r => `${r.assignedZoneIds.length}개` },
  ];
      ...
      <Tabs value={activeTab}>
        ...
        <TabsContent value="info" currentValue={activeTab} className="pt-4">
          <div className="space-y-2">
            <Field label="파트너명"><Input name="partnerName" value={formData.partnerName || ''} onChange={handleInputChange} /></Field>
            <Field label="대표자"><Input name="ceoName" value={formData.ceoName || ''} onChange={handleInputChange} /></Field>
            <Field label="사업자번호"><Input name="businessNumber" value={formData.businessNumber || ''} onChange={handleInputChange} /></Field>
            <Field label="주소"><Input name="address" value={formData.address || ''} onChange={handleInputChange} /></Field>
            <Field label="연락처"><Input name="phone" value={formData.phone || ''} onChange={handleInputChange} /></Field>
            <Field label="이메일"><Input name="email" value={formData.email || ''} onChange={handleInputChange} /></Field>
