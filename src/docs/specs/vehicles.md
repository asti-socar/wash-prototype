...
> **참고 (리스트 미노출 항목)**
> - **파트너 명**, **발행된 오더** 컬럼은 리스트에서 제외됩니다.
> - **사유**: 차량 데이터는 존(Zone) 기준으로 관리되며, 파트너 배정은 존 단위로 이루어집니다. 차량별 파트너 명과 진행 중인 오더를 리스트에서 실시간 조회하려면 존 배정 및 오더 데이터를 차량마다 조인해야 하므로, 대량 데이터 조회 시 성능 저하가 발생합니다.
> - **대안**: 검색/필터 영역의 **파트너 명 필터**를 활용하여 특정 파트너가 담당하는 존의 차량만 조회할 수 있습니다. 개별 차량의 발행된 오더는 **상세 화면**에서 확인합니다.
