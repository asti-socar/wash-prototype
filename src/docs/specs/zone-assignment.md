...
#### C. 파트너 배정 섹션
- **배정 파트너**: Select 드롭다운으로 현재 배정 상태 표시 및 변경
  - 옵션: "미배정" + 전체 파트너 목록
  - 파트너 변경 시 `confirm()` 다이얼로그 표시: "배정 파트너를 '{파트너명}'(으)로 변경하시겠습니까?"
  - 배정된 수행원이 있는 경우 추가 메시지: "배정된 수행원 정보가 초기화됩니다."
  - 확인 시 즉시 반영 (별도 저장 버튼 없음)

#### D. 배정된 수행원 정보 섹션
- 해당 존에 배정된 수행원 목록을 테이블로 표시
- **조건**: 파트너가 배정된 존에서만 수행원 표시
  - 파트너 미배정 존: "파트너가 배정되지 않은 존입니다." 안내 메시지
  - 파트너 배정되었으나 수행원 없는 존: "배정된 수행원이 없습니다." 안내 메시지
- **컬럼**: 수행원 ID, 이름, 벌점
  - 벌점: 0이면 회색, 1 이상이면 빨간 배지
- **수행원 매칭 기준**: 수행원의 소속 파트너와 배정 존이 모두 일치하는 경우
...
### 5.3 파트너 변경 UX
- 파트너 Select 변경 시 즉시 `confirm()` 다이얼로그 표시
- 배정된 수행원이 있는 경우: "배정된 수행원 정보가 초기화됩니다." 추가 안내
- 확인 시 즉시 반영, 취소 시 Select 값 원복
- 별도 저장 버튼 없이 Footer는 [닫기] 버튼만 제공

### 5.4 수행원 정보 표시 정책
- 파트너 미배정 존: 수행원 섹션에 안내 메시지 표시
- 파트너 변경 시: 새 파트너 소속 수행원으로 자동 갱신 (이전 파트너 수행원 초기화)
