...
#### 리드타임 산출 제외 조건
- **파트너 무과실 취소 구간 제외**: 우천 취소, 노쇼 취소 등 파트너 책임이 아닌 사유로 취소된 경우, 해당 취소~재생성 사이 경과 시간은 리드타임 산출에서 제외
- **긴급/장애 재발행 오더 제외**: 긴급 오더 또는 장애로 인해 정규 주기와 별도로 발행된 오더가 완료된 경우, 해당 오더의 리드타임은 파트너 성과 집계에서 제외
- **집계 단위**: 파트너 단위로 완료된 오더 기준 평균 리드타임 산출

### 2.5 오더 지연
발행 후 72시간이 경과하도록 완료되지 않은 오더는 **지연 오더**로 분류됩니다.

- **판단 기준**: 발행 일시 기준 72시간 초과 + 진행 상태가 완료/취소가 아닌 오더
- **대시보드 연동**: 대시보드 "파트너 수행 관리" 영역에서 지연 건수로 집계 (단, 대시보드에서는 경과 7일 초과를 경고 기준으로 사용)
