...
| **필터** | 지역2 | 검색+선택(SearchableSelect) | 선택된 '지역1'에 귀속된 하위 지역(Region2) 목록을 검색 가능한 드롭다운으로 노출. 텍스트 입력 시 옵션이 필터링되며, 목록에서 직접 선택도 가능. <br>※ **지역1 미선택 시 비활성화 (회색 처리)**. |
| **필터** | 파트너 이름 | 선택(Select) | 파트너사 이름 기준 필터링. |
...
- **기본 정렬**: 마지막 세차일 오름차순.
- **정렬(Sort)**: 정렬 가능 컬럼에 한해 헤더 클릭 시 오름차순/내림차순 토글. 정렬 가능 컬럼은 헤더에 화살표 아이콘(↕)으로 표시.
...
| 세차 경과일 | | `오늘 날짜 - 마지막 세차일` (일 단위 계산) |

> **참고 (리스트 미노출 항목)**
> - **파트너 이름**, **발행된 오더** 컬럼은 리스트에서 제외됩니다.
> - **사유**: 차량 데이터는 존(Zone) 기준으로 관리되며, 파트너 배정은 존 단위로 이루어집니다. 차량별 파트너 이름과 진행 중인 오더를 리스트에서 실시간 조회하려면 존 배정 및 오더 데이터를 차량마다 조인해야 하므로, 대량 데이터 조회 시 성능 저하가 발생합니다.
> - **대안**: 검색/필터 영역의 **파트너 이름 필터**를 활용하여 특정 파트너가 담당하는 존의 차량만 조회할 수 있습니다. 개별 차량의 발행된 오더는 **상세 화면**에서 확인합니다.
...
- **지역 정보**: {지역1} / {지역2}
- **파트너 이름**
- **발행된 오더**: `{오더ID} ({상태값})` 형식으로 표시
  - 오더 ID는 **하이퍼링크 스타일**(밑줄 + 새창 아이콘)로 표시
  - 클릭 시 해당 오더 상세 페이지(새 창)로 이동
  - 발행된 오더가 없는 경우 "없음" 표시
...
- UI에는 해당 오더의 **ID**와 현재 **진행 상태**(예: 발행, 예약, 수행 중 등)를 함께 노출하여 진행 단계를 파악할 수 있어야 합니다.
