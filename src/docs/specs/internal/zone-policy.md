...
| 세차관리 | Y/N. 특수 계약 등으로 세차 관리 대상 여부 설정 (존 단위 속성, 정책 상속 없음) | 가능 |
...
| (액션) | | Pencil(수정) + Trash2(초기화) 아이콘 |

### 3.2 상세 (Drawer)
행 클릭 시 상세 정보 조회. [수정] 버튼 클릭 시 편집 모드 전환. Pencil 아이콘 클릭 시 바로 수정 모드로 열림.
- **Drawer 제목**: 상세 시 `정책 상세: {존 이름}`, 수정 시 `정책 수정: {존 이름}`
- **수정 가능 항목**: 존 유형, 세차관리, 주기세차 경과일, 라이트세차, 존 특이사항
- **Footer**:
  - 읽기 모드: [닫기] ... [수정] [삭제]
  - 편집 모드: [취소] [저장]
...
- 저장 클릭 시 변경된 필드를 비교하여 변경 내역(From → To)을 팝업으로 표시
- 변경 사항이 없으면 팝업 없이 편집 모드 종료
- [확인] 클릭 시 저장 반영 후 Drawer 닫힘, [취소] 클릭 시 팝업만 닫힘
- z-index: 1100 (Drawer 위에 표시)

### 3.2.2 삭제 (DeleteConfirmModal)
- 리스트 Trash2 아이콘 또는 상세 Drawer [삭제] 버튼 클릭 시 정책 초기화 확인 팝업 표시
- [초기화] 클릭 시 존 수준 정책(주기세차, 라이트세차)을 초기화하여 상위 정책(지역2/지역1)을 따르도록 변경
...
- 팝업에서 주기세차 경과일 / 라이트세차 / 세차관리 변경 후 저장 → 필터링된 모든 존에 일괄 적용
