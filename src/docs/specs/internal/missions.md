...
| 구분 | 항목 | 타입 | 동작 정의 |
| :--- | :--- | :--- | :--- |
| **검색** | 미션 제목 | 텍스트 검색 | 검색 아이콘 포함. 부분 일치 검색 (입력 텍스트가 제목에 포함되면 매칭) |
| **기간** | 등록 일시 | 날짜 범위 | 시작일 `~` 종료일 한 줄 배치. 범위 조회 |
...
| :--- | :--- |
| **ID** | 정책 고유 식별자 |
| **미션 제목** | 정책의 제목 (`title` 필드, 없을 경우 `content` 필드 표시) |
| **금액** | 3자리 콤마 포맷 (`{금액}원`) |
| **대상 차량수** | **[파생 데이터]** 해당 정책에 할당된 전체 차량 수 (`{N}대`) |
| **수행 완료수** | **[파생 데이터]** 할당된 차량 중 상태가 'completed'인 차량 수 (`{N}대`) |
| **진행률(%)** | **[파생 데이터]** `(수행 완료수 / 대상 차량수) * 100` (반올림 처리) |
| **상태** | **[배지]** 활성(Green) / 비활성(Default) |
| **등록일시** | `yyyy-MM-dd hh:mm` 형식 |
