...
| 파트너 유형 | O | default | |
| (액션) | | | Pencil(수정) + Trash2(삭제) 아이콘 |

### 3.2 상세/등록 (Drawer)
행 클릭 시 상세 조회(읽기 모드), [수정하기] 클릭 시 편집 모드 전환. 신규 등록과 수정을 하나의 Drawer에서 처리. Pencil 아이콘 클릭 시 바로 수정 모드로 열림.

- **Drawer 제목**: 신규 등록 시 `발행 유형 추가`, 상세 조회 시 `[ID: {id}] 발행 유형 정책 상세`, 수정 시 `[ID: {id}] 발행 유형 정책 수정`
- **Footer**: 좌측 버전 표시(`Version: N`, 수정 모드만), 우측 버튼
  - 읽기 모드: [닫기] ... [수정하기] [삭제]
  - 편집 모드: [취소] [저장하기/추가하기]

### 3.3 수정 확인 팝업 (SaveConfirmModal)
- 저장 클릭 시 변경된 필드를 비교하여 변경 내역(From → To)을 팝업으로 표시
- 변경 사항이 없으면 팝업 없이 편집 모드 종료
- [확인] 클릭 시 저장 반영, [취소] 클릭 시 팝업만 닫힘
- z-index: 1100 (Drawer 위에 표시)

### 3.4 삭제 (DeleteConfirmModal)
- 리스트 Trash2 아이콘 또는 상세 Drawer [삭제] 버튼 클릭 시 삭제 확인 팝업 표시
- [확인] 시 해당 행을 리스트에서 제거 (soft delete)
