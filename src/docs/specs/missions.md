...
- **입력 방식**: 텍스트 영역(Textarea)에 검색할 차량번호 입력.
- **구분자 로직**: **콤마(,)** 또는 **줄바꿈(Enter)**을 기준으로 여러 차량번호를 동시에 검색 가능.
- **동작**: 입력된 차량번호와 일치하는 차량만 아래의 '할당된 차량 목록'에 필터링하여 표시.
...
### 3.1 오더 연동 정책
- **자동 포함**: 특정 차량에 대해 세차 오더가 발행될 때, 해당 차량번호가 할당된 '대기' 상태의 미션 정책이 있다면 오더의 미션 목록에 자동으로 포함됩니다.
- **상태 동기화**: 미션 할당된 오더에서, 미션의 수행 상태가 '완료'로 기록되면, 해당 정책 하위의 차량 수행 상태도 '완료'로 자동 업데이트됩니다.
