...
| **검색** | 차량번호(기본), 오더ID, 존 이름, 수행원 기준 검색 |
| **필터** | **기간**: 발행일 기준 (기본: 최근 1개월)<br>**속성**: 지역, 오더 구분, 발행/세차 유형<br>**파트너**: 파트너사 명, 유형(현장/입고), 진행 상태 |
| **필터 동작** | 지역1 변경 시 지역2 자동 초기화<br>파트너 유형 변경 시 유효하지 않은 상태 필터 해제 (예: 현장 선택 시 "입고 중" 해제) |
...
- **상태 배지 색상**: 완료(초록) / 미배정(빨강) / 예약·파트너배정(주황) / 수행관련(파랑) / 취소(회색)
...
| 탭 명 | 주요 내용 및 기능 |
| :--- | :--- |
| **① 상세정보** | - 기본 정보, 시간 정보, 금액, 연계 오더(입고 시)<br>- **[수행 완료]**: 작업 및 점검 종료 처리<br>- **[오더 삭제]**: 오더 폐기 (사유 입력 필수) |
| **② 사진 및 점검** | - **진행 이력**: 상태 변경 타임라인<br>- **사진**: 세차 전/후 사진, 기타 사진<br>- **점검 리스트**: 유형별(A~D) 점검 결과 및 이상 항목 확인 |
| **③ 미션 및 분실물** | - **미션**: 연동된 미션 목록 및 상태<br>- **분실물**: 습득된 물품 정보 |
...
**3. 미배정 모니터링**
- 필터 > 진행상태 "미배정" 선택 → 목록에서 파트너 미배정 오더 확인

#### 예외 및 주의사항 (FAQ)
-   **차량 조회 실패**: 시스템 미등록 차량은 오더 발행 불가 (차량 등록 선행 필요).
-   **파트너 유형과 상태**: '현장' 파트너 선택 시 '입고/출고' 관련 상태 필터는 자동 해제됨.
-   **오더 삭제**: 삭제 시 복구 불가하며, 반드시 사유를 입력해야 함 (감사 로그 기록).
...
| **차량 정보** | 차량ID, 번호, 차종, 존(ID/이름), 지역(1/2) | 오더 발행 시 실시간 조회 |
| **파트너** | 파트너 명, 유형(현장/입고), 수행원 | 유형 기본값: 현장 |
| **상세 데이터** | 메모, 사진(전/후/기타), 점검결과(JSON), 미션(Snapshot) | |
...
파트너 유형에 따라 유효한 상태 흐름이 다릅니다.

-   **현장 파트너**: `미배정` → `예약` → `수행 중` → `완료` (또는 취소)
-   **입고 파트너**: `미배정` → `파트너 배정` → `예약` → `입고 중` → `수행 중` → `세차 완료` → `출고 중` → `완료` (또는 취소)

*Validation*: 필터링 시 파트너 유형 변경 시 유효하지 않은 상태 값 자동 초기화
- 예: "입고" → "현장" 변경 시, "입고 중", "세차 완료", "출고 중" 필터 해제
...
#### 오더 삭제 및 완료
-   **삭제**: [삭제] 버튼 → 사유 입력 팝업(필수) → 확정 시 Soft Delete 및 감사 로그 적재
